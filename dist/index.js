...
var PolkadotApiService = class _PolkadotApiService {
    ...
    for (let attempt = 0; attempt < config.MAX_RETRIES; attempt++) {
      for (const endpoint of endpoints) {
        try {
          ...
          const apiConfig = _PolkadotApiService.getApiConfig(networkType, provider);
   ...
   */
  static getApiConfig(networkType, provider) {
    const baseConfig = { provider };
    if (networkType === "asset-hub" /* ASSET_HUB */) {
      return {
        ...baseConfig,
        signedExtensions: {
...
// src/actions/subsidizedTransfer.ts
import {
  elizaLogger,
...
// src/actions/subsidizedTransfer.ts
var subsidizedTransferSchema = z16.object({
  recipientAddress: z16.string(),
  assetId: z16.string(),
  amount: z16.string(),
  feeAssetId: z16.string(),
  walletNumber: z16.number().optional().nullable(),
  walletAddress: z16.string().optional().nullable(),
  password: z16.string().optional().nullable()
});
var subsidizedTransferTemplate = `Respond with a JSON markdown block containing only the extracted values. Use null for any values that cannot be determined.
  Example response:
  \`\`\`json
  {
    "recipientAddress": "<recipient address>",
  ...
  Respond with a JSON markdown block containing only the extracted values.`;
async function buildSubsidizedTransferDetails(runtime, message, state) {
  const currentState = state || await runtime.composeState(message);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    parsedResponse = parseJSONObjectFromText13(response);
    if (parsedResponse) {
      break;
    }
  }
...
var SubsidizedTransferAction = class {
  runtime;
  walletProvider;
  constructor(runtime) {
    this.runtime = runtime;
    ...
    const feeAssetLocation = {
      ...
      interior: {
        X2: [
          { palletInstance: 50 },
          // Assets pallet
          { generalIndex: parseInt(params.feeAssetId) }
          // Fee asset ID
        ]
      }
    };
...
var subsidizedTransfer_default = {
  ...
  description: "Transfers assets on Asset Hub while paying transaction fees with a different sufficient asset (e.g., pay fees with USDC instead of DOT).",
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (!transferContent || !transferContent.recipientAddress || !transferContent.assetId || !transferContent.amount || !transferContent.feeAssetId) {
...
export {
  ...
  subsidizedTransfer_default as SubsidizedTransfer,
